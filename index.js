...
app.get('/territorios/export/pdf', async (req, res) => {
  try {
    // 1. Buscamos e agrupamos os dados (lógica existente)
    const historicoQuery = `
      SELECT t.id, t.numero, t.descricao, p.nome as pessoa_nome, d.data_saida, d.data_devolucao
      FROM territorios t
      LEFT JOIN designacoes d ON t.id = d.territorio_id
      LEFT JOIN pessoas p ON d.pessoa_id = p.id
      ORDER BY CAST(NULLIF(regexp_replace(t.numero, '[^0-9]', '', 'g'), '') AS INTEGER) ASC, d.data_saida ASC;
    `;
    const result = await pool.query(historicoQuery);

    const territoriosMap = new Map();
    result.rows.forEach(row => {
      if (!territoriosMap.has(row.id)) {
        territoriosMap.set(row.id, {
          id: row.id, numero: row.numero, descricao: row.descricao, historico: []
        });
      }
      if (row.pessoa_nome) {
        territoriosMap.get(row.id).historico.push(row);
      }
    });
    const reportData = Array.from(territoriosMap.values());

    // 2. Lógica para calcular o Ano de Serviço
    const hoje = new Date();
    const ano = hoje.getFullYear();
    const mes = hoje.getMonth(); // 0 = Janeiro, 8 = Setembro
    const anoDeServico = mes >= 8 ? ano + 1 : ano;

    // 3. Geramos as linhas da tabela (<tbody>)
    let tableRows = '';
    reportData.forEach(territorio => {
      const historicoReverso = [...territorio.historico].reverse(); // Queremos o mais recente primeiro na coluna
      const lastReturn = historicoReverso.find(h => h.data_devolucao);
      const lastReturnDate = lastReturn ? new Date(lastReturn.data_devolucao).toLocaleDateString('pt-BR') : '---';
      
      let historyCells = '';
      for (let i = 0; i < 4; i++) {
        const reg = territorio.historico[i]; // Pegamos os registros mais antigos primeiro para preencher da esquerda para a direita
        if (reg) {
          historyCells += `
            ...
            <td class="history-cell">
              ${new Date(reg.data_saida).toLocaleDateString('pt-BR')}
            </td>
            <td class="history-cell">
              ${reg.data_devolucao ? new Date(reg.data_devolucao).toLocaleDateString('pt-BR') : '...'}
            </td>
          `;
        } else {
          // Preenche com 3 células vazias se não houver registro
          historyCells += '<td class="history-cell"></td><td class="history-cell"></td><td class="history-cell"></td>';
        }
      }
      
      tableRows += `
        <tr>
          <td class="terr-cell"><b>${territorio.numero}</b><br><small>${territorio.descricao || ''}</small></td>
          <td class="date-cell">${lastReturnDate}</td>
          ${historyCells}
        </tr>
      `;
    });

    // 4. Montamos o HTML completo com o novo cabeçalho e CSS
    const htmlContent = `
      <html>
        <head>
          <style>
            ...
            .service-year { font-size: 12px; }
            table { width: 100%; border-collapse: collapse; }
            th, td { border: 1px solid black; padding: 3px; text-align: center; vertical-align: middle; }
            th { font-size: 10px; background-color: #f2f2f2; }
            .terr-cell { width: 10%; text-align: left; vertical-align: top; }
            .date-cell { width: 8%; }
            .history-cell { width: 6.8%; word-wrap: break-word; }
        ...
        <body>
          <div class="header">
            ...
            <div class="service-year">Ano de serviço: ${anoDeServico}</div>
          </div>
          <table>
            <thead>
              <tr>
                <th rowspan="2" style="vertical-align: middle;">Terr. n.º</th>
                <th rowspan="2" style="vertical-align: middle;">Última data concluída*</th>
                <th colspan="3">Designado para</th>
                <th colspan="3">Designado para</th>
                <th colspan="3">Designado para</th>
                <th colspan="3">Designado para</th>
              </tr>
              <tr>
                <th>Designado para</th><th>Data da designação</th><th>Data da conclusão</th>
                <th>Designado para</th><th>Data da designação</th><th>Data da conclusão</th>
                <th>Designado para</th><th>Data da designação</th><th>Data da conclusão</th>
                <th>Designado para</th><th>Data da designação</th><th>Data da conclusão</th>
              </tr>
    ...
    // 6. Enviamos o PDF para o navegador
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', `attachment; filename="S-13_Relatorio_${new Date().toISOString().split('T')[0]}.pdf"`);
